...
class Antibody(object):
    def __init__(
        ...
        self.dominant_fitness_check = lambda: np.max(
            [self.entropy_fitness, self.distance_fitness]
        )
    ...
    def calculate_entropy(self) -> float:
        ...
        for i in range(len(self.grey_level_histograms)):
            entropy_sum += -np.sum(
                self.grey_level_histograms[i]
                * np.ma.log2(self.grey_level_histograms[i])
            )
    ...
    def calculate_distance(self) -> float:
        ...
        for i in range(self.grey_level_histograms.__len__()):
            for j in range(i + 1, self.grey_level_histograms.__len__()):
                distances += -np.sum(
                    self.grey_level_histograms[i]
                    * np.ma.log2(self.grey_level_histograms[j])
                ) + -np.sum(
                    self.grey_level_histograms[j]
                    * np.ma.log2(self.grey_level_histograms[i])
                )
        return float(
            (2 / (self.designed_band_size * (self.designed_band_size - 1))) * distances
        )
