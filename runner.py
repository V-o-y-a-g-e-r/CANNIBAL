...
def get_bombs_bands(n_clusters) -> List:
    return (
        np.loadtxt(f"bombs/pavia_out_{n_clusters}/best_individual_bands_{n_clusters}")
        .astype(int)
        .tolist()
    )
...
def get_mi_bands(n_clusters) -> List:
    return np.loadtxt(f"mi/pavia_out{n_clusters}/chosen_bands").astype(int).tolist()
...
def run(
    ...
    start_time = time()
    X_train, X_test, y_train, y_test, test_indices, tr_i = load_train_test_data()
    ...
    f1_weighted = f1_score(
        y_true=y_test, y_pred=y_test_pred, average="weighted"
    )
    ...
    avg_acc = balanced_accuracy_score(y_true=y_test, y_pred=y_test_pred)

    class_metrics = {"acc": acc, "avg_acc": avg_acc, "f1_micro": f1_micro, "f1_macro": f1_macro, "f1_weighted": f1_weighted,
    ...
    for k, v in class_metrics.items():
        class_metrics[k] = [v.item()]

    class_metrics["time"] = time() - start_time
    class_metrics = dict(sorted(class_metrics.items()))

    # dir_name = f"knn_all_pavia_results_test_frac_band_sel_estimators_4"
    # if not os.path.isdir(dir_name):
    #     os.makedirs(dir_name, exist_ok=True)
    # pd.DataFrame.from_dict(class_metrics).to_csv(
    ...
    #         fmt="%d",
    #     )
    return test_indices, y_test_pred, tr_i
...
def load_train_test_data():
    ...
    y_test = y_test[mask]
    X_test = X_test[mask]
    test_indices = test_indices[mask]
...
if __name__ == "__main__":
    ...
    # arguments_list = []
    # for n_clusters in tqdm(list(range(5, 103, 5))):
