...
from bombs.antibody_individual import Antibody
from bombs.utils import (
    load_data,
...
class AntibodyPopulation(object):
        ...
        [
            self.P.append(
                Antibody(
                    ...
                    ref_map_path=self.args.ref_map_path,
                )
            )
            for selected_bands, band_indexes in bands
        ]
    ...
    def update_dominant_population(self):
        ...
        if self.P.__len__() == self.args.Nd:
            ...
            if self.args.TD_size > self.args.Nd:
                chosen_antibodies = calculate_crowding_distances(
                    list_of_antibodies=self.TD
                )
                chosen_antibodies = np.argsort(-chosen_antibodies).tolist()[
                    : self.args.Nd
                ]
    ...
    def _copy(self, band_indexes: list) -> Antibody:
        ...
        """
        return Antibody(
            selected_bands=load_data(self.args.data_path, self.args.ref_map_path)[
                ..., np.unique(band_indexes)
            ],
            band_indexes=band_indexes,
            data_path=self.args.data_path,
            ref_map_path=self.args.ref_map_path,
    ...
    def heterogeneous_mutation(self, generation_idx: int):
        ...
        assert self.u_max is not None, (
            "Value of maximum number of bands,"
        ...
        for individual_index in range(self.C_prime.__len__()):
            ...
            for band_index in range(self.C_prime[individual_index].designed_band_size):
                ...
                if rand < mutation_prob:
                    z_parameter = (
                        self.u_max
                        - self.C_prime[individual_index].band_indexes[band_index]
                    )
                    mutation_scope = self.calculate_mutation_scope(
                        z_parameter=z_parameter,
                        generation_idx=generation_idx,
                        alpha=alpha,
                    )
                    self.C_prime[individual_index].band_indexes[
                        band_index
                    ] += mutation_scope
    ...
    def whole_arithmetic_crossover(self):
        ...
        while len(self.C_prime) < self.args.Nc:
            a_i, c_j = (
                np.random.choice(self.A, size=1)[0],
                np.random.choice(self.C, size=1)[0],
            )
    ...
    def proportional_cloning(self):
        ...
        """
        active_crowding_distances = calculate_crowding_distances(
            list_of_antibodies=self.A
        )
        cloning_times = [
            int(
                math.ceil(
                    self.args.Nc * (crowding_value / sum(active_crowding_distances))
                )
            )
    ...
    def show_bands(self):
        ...
        print(
            "Bands of the best individual in the whole population: {}".format(
                np.sort(self.D[max_].band_indexes)
            )
        )
        print(
            "Entropy: {}".format(self.D[max_].entropy_fitness),
    ...
    def calculate_mutation_scope(
        self, z_parameter: int, generation_idx: int, alpha: float
    ) -> int:
        """
    ...
    def serialize_individuals(self):
        ...
        np.savetxt(
            os.path.join(
                self.args.dest_path,
                f"best_individual_bands_{self.args.bands_per_antibody}",
            ),
            np.sort(np.unique(self.D[max_i].band_indexes)),
            fmt="%d",
        )
        np.savetxt(
            os.path.join(
                self.args.dest_path,
                f"best_individual_entropy_{self.args.bands_per_antibody}",
            ),
            np.sort(np.unique(self.D[max_i].entropy_fitness)),
            fmt="%5.5f",
        )
        np.savetxt(
            os.path.join(
                self.args.dest_path,
                f"best_individual_distance_{self.args.bands_per_antibody}",
            ),
            np.sort(np.unique(self.D[max_i].distance_fitness)),
            fmt="%5.5f",
        )
    ...
    def nondominated_sort(self) -> list:
        ...
        sorted_list = [
            antibody for sub_fronts in non_dominated_frotns for antibody in sub_fronts
        ]
        return sorted_list
    ...
    def randomize_bands(self, args) -> list:
        ...
        for i in range(args.P_init_size):
            chosen_bands = np.random.choice(
                a=self.u_max, size=args.bands_per_antibody, replace=False
            )
